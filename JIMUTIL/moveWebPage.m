...
% move a webpage from the current directory. 
% it assumes that you have already placed a copy in the destination, and now you 
% simply want an auto rediurection for all .htm and .html files. 
% SYNTAX:     moveWebPage(destination,recurseDirs,verbose)
...
function doWebPage(destination,recurseDirs,verbose);

% ============ SETUP ============
prog_name='moveWebPage.m';                              % this program's name
fprintf('\n~ %s ~  ',prog_name)                         % orient user


if ~exist('doList'), str2mat('.html','.htm'); end       % if not passed
...
H=str2mat('<html>',                                 ... % 
  '<! created using moveWebPage.m by Jim Patton>',    ... %
  '<font face="Arial">' ,                           ... % 
  ['<h1>' dirName '</h1>'],                         ... % 
  ['<p>'],                                          ... %
...
S=' ';
Bflag=0;                                                % init false; indicates biblio info
Dflag=0;                                                % init false; indicates subdirectories
if ~isempty(picWidth), 
...
S=str2mat(H,D,'<hr>',S,['<hr><small> <i>(Last updated '... %
