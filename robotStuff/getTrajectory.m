...
function [rho,q,phi,t1]=getTrajectory(trajFileName,L,rot,mag,cntrPt,verbose);

%_______ SETUP ______
progname='getTrajectory.m';
...
phi=inverse_kinematics3(rho(:,2:3),0,L);    % desired joint (relative) ang 
[phi(:,3:4),phi(:,5:6)]=dbl_diff(phi,Hz,0); % calc higher derivatives
phi=[t1 phi];                               % add a time col
  
%___ CALC q (seg angles & deriv's) __
q=inverse_kinematics3(rho(:,2:3),1,L);      % desired joint (relative) ang 
