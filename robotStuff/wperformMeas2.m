...
while(1)
  ...
  for trial=1:size(trialData,1) % trial 79 is a perturb?
    ...
    if BLSt>size(BL,1)-aFrames, 
      fprintf(' (no mvmnt in baseline!) ',DSt)
      BLSt=1;
    end       % if bad, use 1st frame past 5 mm
    ...
    speed_D=sqrt(D(:,4).^2+D(:,5).^2);                                  % calulate speed
    ...
    if 0 % for debug
      trial, part,  FigErr,        InfNorm,         InitDirErr      ... % stack into EM matrix
