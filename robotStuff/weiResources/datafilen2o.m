...
function datafilen2o(ndatafile,movementtype,starttrial)

% ndatafile = 'D:\Patton_Shared_Data\sfd\sfd128\sfd.dat';
 ...
 for j=starttrial:length(counttrial)
      vdata = [ ];
       if j<length(counttrial)
          [vdata,Moved] = add_vel(data, [counttrial(j,1) counttrial(j,2)]);
        else
          [vdata,Moved] = add_vel(data, [counttrial(j,1) length(data)-20]);
        end
